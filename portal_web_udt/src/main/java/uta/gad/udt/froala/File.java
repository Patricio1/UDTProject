...
public final class File {
	...
	public static void delete(HttpServletRequest req, String src) {
		...
		String []auxnombreArchivo = filePath.split("/");
		int indiceUltimo = (auxnombreArchivo.length>0)?auxnombreArchivo.length-1:0;
		String pathModificado ="";
		for (int i = 0; i < auxnombreArchivo.length-3; i++) {
							
						pathModificado+=auxnombreArchivo[i]+"/";
			
		}
		pathModificado+="WEB-INF/uploads/"+auxnombreArchivo[indiceUltimo];
		System.out.println("ABSOLUTE PATH DELETE: "+filePath);
		System.out.println("ABSOLUTE PATH MODY. DELETE: "+pathModificado);
		

		java.io.File file = new java.io.File(pathModificado);
		if (file.exists()) {
			file.delete();
		}
	}
